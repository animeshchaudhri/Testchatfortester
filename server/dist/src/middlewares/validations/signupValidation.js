...
var signupSchema = yup.object().shape({
    name: yup
        .string()
        .transform(function (value) { return (value !== null ? value.charAt(0).toUpperCase() + value.slice(1) : value); })
        .trim()
        .required('Name can not be empty')
        .test('isPerfectString', 'Enter a valid name', function (arg) { return /^[A-Za-z ]+$/.test(arg); }),
    username: yup
        .string()
        .trim()
...
var signupValidation = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var validationErrors, _a, name, username, email, password, _b, existingUser, existingUsername, validatedData, err_1;
        ...
        switch (_c.label) {
            ...
            case 1:
                ...
                return [4 /*yield*/, Promise.all([
                        prismadb_1.default.user.findFirst({ where: { email: email } }),
                        prismadb_1.default.user.findFirst({ where: { username: username } }),
                    ])];
            case 2:
                _b = _c.sent(), existingUser = _b[0], existingUsername = _b[1];
                if (existingUser) {
                ...
                return [4 /*yield*/, signupSchema.validate({ name: name, username: username, email: email, password: password }, { stripUnknown: true, abortEarly: false })];
            case 3:
                validatedData = _c.sent();
